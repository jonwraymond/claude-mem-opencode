...
export const ClaudeMemPlugin = async (ctx) => {
    ...
    config.debug && config.debug && console.log(`[CLAUDE_MEM] Configuration loaded from: ${sources.join(' → ')}`);
    ...
    config.debug && config.debug && console.log('[CLAUDE_MEM] Initializing plugin...');
    config.debug && config.debug && console.log(`[CLAUDE_MEM] Project: ${projectName}`);
    config.debug && config.debug && console.log(`[CLAUDE_MEM] Worker port: ${config.workerPort}`);
    config.debug && config.debug && console.log(`[CLAUDE_MEM] Debug: ${config.debug}`);
    // Initialize worker client
    const workerClient = new WorkerClient(config.workerPort);
    // Fail hard: Worker must be available
    try {
        config.debug && config.debug && console.log('[CLAUDE_MEM] Checking worker availability...');
        ...
        config.debug && config.debug && console.log('[CLAUDE_MEM] ✅ Worker connected - plugin active');
    }
    catch (error) {
    ...
    return {
        ...
        'chat.message': async (input, output) => {
            ...
            try {
                config.debug && config.debug && console.log(`[CLAUDE_MEM] Injecting context for session ${input.sessionID}`);
                ...
                if (!response.success || !response.results || response.results.length === 0) {
                    config.debug && config.debug && console.log('[CLAUDE_MEM] No relevant memories found');
                ...
                state.contextInjected = true;
                config.debug && config.debug && console.log(`[CLAUDE_MEM] ✅ Context injected (${contextText.length} chars)`);
            }
            catch (error) {
        ...
        'tool.execute.after': async (input, output) => {
            ...
            try {
                config.debug && config.debug && console.log(`[CLAUDE_MEM] Capturing tool: ${input.tool}`);
                ...
                config.debug && config.debug && console.log(`[CLAUDE_MEM] ✅ Observation added: ${input.tool}`);
            }
            catch (error) {
                    ...
                    try {
                        ...
                        switch (args.mode || 'search') {
                            case 'search': {
                                ...
                                config.debug && config.debug && console.log(`[CLAUDE_MEM] Searching: ${args.query}`);
...
async function handleSessionCreated(event, sessionStates, workerClient, projectName, privacyStripper) {
    ...
    if (isPrivate) {
        config.debug && config.debug && console.log(`[CLAUDE_MEM] Skipping private session: ${sessionId}`);
    ...
    try {
        ...
        if (response.skipped) {
            config.debug && config.debug && console.log(`[CLAUDE_MEM] Session marked as private: ${sessionId}`);
            config.debug && config.debug && console.log(`[CLAUDE_MEM] Reason: ${response.reason}`);
            sessionStates.set(sessionId, {
                sessionId,
                claudeMemSessionId: null,
                contextInjected: true,
        ...
        config.debug && config.debug && console.log(`[CLAUDE_MEM] Session ${sessionId} → ${response.sessionDbId}`);
    }
    catch (error) {
...
async function handleSessionUpdated(event, sessionStates, workerClient) {
    ...
    try {
        config.debug && config.debug && console.log(`[CLAUDE_MEM] Completing session: ${sessionId}`);
        await workerClient.completeSession(state.claudeMemSessionId);
        config.debug && config.debug && console.log(`[CLAUDE_MEM] ✅ Session completed: ${state.claudeMemSessionId}`);
        // Clean up state
